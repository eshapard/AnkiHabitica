...
for f in ["Anki_HRPG.py", "ankiHRPG.py", "ankiHRPG_v0.5.py", "ankiHRPG_v1.0.py", "ankiHRPG_v1.2.py"]:
    ...
    if os.path.exists(old_version_file):
        ...
        if delete_me:
            if ah.settings.keep_log:
                ah.log.info("Deleting old version of add-on: %s" %
                            old_version_file)
    ...
    if not ah.settings.import_rejected and ah.old_config_exists and not ah.settings.configured:
        if read_OLD_conf_file(ah.old_conffile):
            ...
            if utils.askUser("I can't find any Habitica credentials for this profile (%s), but I did find an old config file for an older version of ankiHRPG.\n\nWould you like to import these credentials?" % ah.settings.profile):
                ...
                if ah.settings.keep_log:
                    ah.log.debug(
                        "OK, I imorted the userID and APItoken from the old config file. Would you like to delete the old configuration file?")
                ...
                if ah.settings.keep_log:
                    ah.log.debug(
                        'not importing settings from old conf file per user response')
...
def calculate_db_score(start_date):
    ...
    if ah.settings.tries_eq:
        dbwrong = int(db_helper.wrong_answer_count(
            start_date) / ah.settings.tries_eq)
    else:
        dbwrong = 0
    if ah.settings.timeboxpoints:
        dbtimebox = int(db_helper.timebox_count(
            start_date) * ah.settings.timeboxpoints)
    else:
        dbtimebox = 0
    if ah.settings.deckpoints:
        dbdecks = int(db_helper.decks_count(
            start_date) * ah.settings.deckpoints)
    else:
        dbdecks = 0
    if ah.settings.learned_eq:
        dblearned = int(db_helper.learned_count(
            start_date) / ah.settings.learned_eq)
    else:
        dblearned = 0
    if ah.settings.matured_eq:
        dbmatured = int(db_helper.matured_count(
            start_date) / ah.settings.matured_eq)
    else:
        dbmatured = 0
    ...
    if ah.settings.configured:
        ...
        if real_point_length >= int(ah.settings.threshold / ah.settings.step):
            extra = real_point_length - \
                int(ah.settings.threshold / ah.settings.step)
        else:
            extra = 0
