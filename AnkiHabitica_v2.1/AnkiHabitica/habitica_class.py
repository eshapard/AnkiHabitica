...
#     debug = False
    ...
    def save_avatar(self):
        ...
        try:
            ...
            if not pngfile: 
                if ah.settings.keep_log: ah.log.error("End function returning: %s" % False) #Exit if we failed
    ...
    def hrpg_showInfo(self, text):
    	if ah.settings.keep_log: ah.log.debug("Begin function")
        if ah.settings.keep_log: ah.log.info("Msg: %s" % text.replace('\n',' '))
    ...
    def update_stats(self, silent=False):
        ...
        if ah.settings.keep_log: ah.log.debug(self.name)
    ...
    def check_anki_habit(self, habit):
        ...
        if ah.settings.keep_log: ah.log.debug("checking %s" % habit)
        ...
        if not found:
            try:
                ...
                if ah.settings.keep_log: ah.log.debug(json.dumps(tasks)) 
                for t in tasks:
                ...
                if found:
                    ...
                    if ah.settings.keep_log: ah.log.debug("Task found")
                else:
                    self.missing[habit] = True
                    #if Habitica.debug: utils.showInfo("Task not found")
                    if ah.settings.keep_log: ah.log.warning("Task not found")
    ...
    def create_missing_habit(self, habit):
        ...
        try:
            ...
            if ah.settings.keep_log: ah.log.debug("New habit created: %s" %self.habit_id[habit])
            #if Habitica.debug: utils.showInfo(json.dumps(msg))
            if ah.settings.keep_log: ah.log.debug(json.dumps(msg))
    ...
    def reset_scorecounter(self, habit):
        ...
        if ah.settings.keep_log: ah.log.debug("Resetting Scorecounter")
        last_review_time = db_helper.latest_review_time()
        #if Habitica.debug: utils.showInfo(str(last_review_time))
        if ah.settings.keep_log: ah.log.debug(str(last_review_time))
        ...
        if ah.settings.keep_log: ah.log.debug("reset: %s" % json.dumps(self.hnote[habit]))
        try:
    ...
    def grab_scorecounter(self, habit):
        ...
        if 'scoresincedate' not in self.hnote[habit] or 'scorecount' not in self.hnote[habit]:
            ...
            if ah.settings.keep_log: ah.log.debug("scorecounter missing keys")
            self.reset_scorecounter(habit)
            if ah.settings.keep_log: ah.log.warning("End function returning: %s" %  False)
            return False
        ...
        if ah.settings.keep_log: ah.log.debug("Habit Grabbed")
        if ah.settings.keep_log: ah.log.debug("Habit note: %s" % self.hnote[habit])
        self.habit_grabbed[habit] = True
        if ah.settings.keep_log: ah.log.debug("End function returning: %s" %  True)
        return True
    ...
    def post_scorecounter(self, habit):
        ...
        try:
            ...
            if ah.settings.keep_log: ah.log.debug("posting scorecounter: %s" % self.hnote[habit])
...
#             self.hrpg_tooltip("Huzzah! You Scored Points!")
    ...
    def earn_points(self, habit):
        ...
        if 'lvl' not in self.stats:
            ...
            if ah.settings.keep_log: ah.log.warning("lvl not in stats")
            self.update_stats(False)
        #check habit if is is unchecked
        if not self.habit_grabbed[habit]:
            #if Habitica.debug: utils.showInfo("%s habit not checked" % habit)
            if ah.settings.keep_log: ah.log.debug("%s habit not checked" % habit)
            try:
                #if Habitica.debug: utils.showInfo("Checking Habit Score Counter")
                if ah.settings.keep_log: ah.log.debug("Checking Habit Score Counter")
                self.check_anki_habit(habit)
            except:
                pass
        ...
        if not success: #exit if we failed all 3 times
                ...
                if ah.settings.keep_log: ah.log.warning('Internet failed')
                if ah.settings.keep_log: ah.log.warning("End function returning: %s" %  False)
                return False
    ...
    def compact_habitica_stats(self):
        ...
        if self.ht and self.xt and self.mt:
            ...
            string = False
        if ah.settings.keep_log: ah.log.debug("End function returning: %s" %  string)
        return string
