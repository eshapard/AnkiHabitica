...
def setup():
	global config, hrpg_score, hrpg_tries, hrpg_habit_configured
...
#Get HRPG User Info
def get_user():
	global hrpg_lvl, hrpg_xp, hrpg_gp, hrpg_hp, hrpg_mp, hrpg_stats, hrpg_name
	response = json.load(urllib2.urlopen(user_object))
	hrpg_name = response['profile']['name']
	utils.showInfo("Welcome %s!\nYour Anki session is now connected to Habitica." % (hrpg_name))
